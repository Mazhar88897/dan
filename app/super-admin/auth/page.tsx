...
const SUPER_ADMIN_EMAIL = "mk0906145@gmail.com";
...
export default function SuperAdminSignInPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (normalizedEmail !== SUPER_ADMIN_EMAIL.toLowerCase()) {
      setError("Only the designated super admin may sign in here.");
      return;
    }
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ id: email.trim(), password }),
      });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#050515] via-[#2a2a2a] to-[#050515] text-white flex items-center justify-center px-4">
      <div className="relative w-full max-w-md">
        ...
        <div className="relative rounded-3xl bg-slate-950/90 border border-white/10 shadow-2xl p-8">
          ...
          <p className="text-sm text-gray-300 mb-6">
            Sign in with the authorized super admin account to manage the platform.
          </p>
          ...
          <p className="mt-4 text-[11px] text-gray-500 text-center">
            Use the same credentials as the main app. Access is restricted to the super admin
            email only.
          </p>
        </div>
      </div>
